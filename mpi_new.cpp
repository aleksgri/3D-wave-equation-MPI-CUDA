...
void prepare_layer(int n) {
    ...
    for(int j = coords[1] == 0 ? 2 : 1; j <= y; j++) {
        for(int k = coords[2] == 0 ? 2 : 1; k <= z; k++) {
            if(coords[0] == dim[0]-1) grids[n % 3].set(X, j, k, (n > 1 ? 2 : 1)*grids[(n + 2) % 3].get(X, j, k) - (n > 1 ? grids[(n + 1) % 3].get(X, j, k) : 0) + (n > 1 ? 1.0 : 0.5)*a2*tau*tau*grids[(n+2) % 3].laplace(X, j, k, X-1, j-1, k-1, X+1, j+1, k+1));
...
int main(int argc, char *argv[]) { // command line args: N, Np, Lx, Ly, Lz, T, timesteps
    ...
    // num_sol.set(1, 0, 0, 0, 1000.0);
    // num_sol.set(1, 6, 6, 6, 1000.0);
