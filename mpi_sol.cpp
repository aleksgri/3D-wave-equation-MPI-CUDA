...
void prepare_layer(int n) { // граничные значения
    ...
    for(int j = coords[1] == 0 ? 2 : 1; j <= y; j++) {
        for(int k = coords[2] == 0 ? 2 : 1; k <= z; k++) {
            if(coords[0] == dim[0]-1) num_sol.set(n, X, j, k, (n > 1 ? 2 : 1)*num_sol.get(n-1, X, j, k) - (n > 1 ? num_sol.get(n-2, X, j, k) : 0) + (n > 1 ? 1.0 : 0.5)*a2*tau*tau*num_sol.laplace(n-1, X, j, k, X-1, j-1, k-1, X+1, j+1, k+1));
            if(coords[0] == 0) num_sol.set(n, 1, j, k, (n > 1 ? 2 : 1)*num_sol.get(n-1, 1, j, k) - (n > 1 ? num_sol.get(n-2, 1, j, k) : 0) + (n > 1 ? 1.0 : 0.5)*a2*tau*tau*num_sol.laplace(n-1, 1, j, k, 0, j-1, k-1, 2, j+1, k+1));
        }
    }
}
...
int main(int argc, char *argv[]) { // command line args: N, Np, Lx, Ly, Lz, T, timesteps
    ...
    abs_error.init_grid(timesteps+2, X, Y, Z);
    rel_error.init_grid(timesteps+2, X, Y, Z);
